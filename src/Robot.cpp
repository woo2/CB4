...
private:
    ...
    void AutonomousPeriodic ()	//Welcome to the Realm of Hearsay. OH! and Elijah.
    {
        ...
        if (auto_server == 0) {				// E I wrote my own Auto Selectors this in theory should let us change auto.
        ...
        else if (auto_server == 1) {

            /* PLEASE CONSIDER AN AND STATEMENT HERE*/

            //Auto code 1 For straight in front of low bar
            ...
            /* Was this if supposed to do bitmath or AND the two args?? */
            if ((r_enc / 360 <= Auto1_F) & (l_enc / 360 <= Auto1_F) /*Maghere straight */ ) {
                //so if rwheel encoder is <= rotations and wheel endoder is <= rotations DRIVE MAN DRIVE
	            rightgo = .7;
	            leftgo = .7;
	        ...
	        SmartDashboard::PutString ("What's new?",
				   "MOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOO and stuffffffffff!");
	        ...
	        SmartDashboard::PutString ("HEY!!!",
				   "You know it is only 1-2 right?");
	        rightgo = 0;
	        leftgo = 0;
	        leds1->Set (1);
        }	//END AUTO 3
    ...
    imaqError =
      IMAQdxOpenCamera ("cam3", IMAQdxCameraControlModeController, &session);
  ...
  void TeleopPeriodic ()
  {
    ...
    if (button_led) {
        ...
        reflectedLight = 0.0;
        //DrC, the phase sensitive detection signal goes in this variable

        for (i = 1; i < samples; i++){
            //DrC basic flash sequence
	        led1->Set (1);
    ...
    imaqDrawShapeOnImage (frame, frame,
			  {
			  10, 10, 100, 100}
